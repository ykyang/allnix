...
public class LongLine extends JPanel implements ActionListener 
{
  ...
  public LongLine() 
  {
    ...
    // > These two lines need to be moved as shown below. 
    //    renWin.resetCamera();
    ...
    SwingUtilities.invokeLater(new Runnable()
      {
        public void run() 
        {
        	LongLine longLine = new LongLine();
          JFrame frame = new JFrame("Long Line");
          frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
          frame.getContentPane().setLayout(new BorderLayout());
          frame.getContentPane().add(longLine, BorderLayout.CENTER);
          frame.setSize(400, 400);
          frame.setLocationRelativeTo(null);
          frame.setVisible(true);
          
          // > Reset Camera < //
          longLine.renWin().GetRenderer().ResetCamera();
          longLine.renWin().resetCameraClippingRange();
        }
      });
  }
}  
